#!/usr/bin/env python3
"""Rank manifest-length source cuts by foreground activity and timbral change."""
...
def best_offset(path: Path, clip_seconds: float) -> tuple[float, float, float, float]:
    measurements = features(path)
    window_frames = max(2, round(clip_seconds / FRAME_SECONDS))
    maximum_start = min(
        len(measurements) - window_frames,
        math.floor((duration(path) - clip_seconds) / FRAME_SECONDS),
    )
    if maximum_start < 0:
        raise ValueError(f"{path} is shorter than {clip_seconds} seconds")
...
def main() -> None:
    ...
    print("id\told_trim_start\ttrim_start\tscore\tlevel_spread_db\tspectral_motion")
    recommendations: dict[str, str] = {}
    def rank(row: dict[str, str]) -> tuple[dict[str, str], float, float, float, float]:
        offset, score, level_spread, spectral_motion = best_offset(
            args.raw_dir / f"{row['id']}.media",
            float(row["seconds"]),
        )
        return row, offset, score, level_spread, spectral_motion

    with concurrent.futures.ThreadPoolExecutor(max_workers=args.jobs) as executor:
        results = list(executor.map(rank, all_rows))
    for row, offset, score, level_spread, spectral_motion in results:
        print(
            f"{row['id']}\t{row['trim_start']}\t{offset:.1f}\t{score:.3f}\t"
