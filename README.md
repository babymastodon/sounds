...
- [`conv8`](conv8/README.md) turns `conv7` into a controlled additive-pitch playground. Deterministic hashes select one of 13 naturally detuned 13-EDO chords and one of three aggressively ruined, power-matched synthesized instruments for every short/long pair, convolution-domain gain calibration keeps their tone stem audible, and two matched matrices compare notes added to the long input versus the short input.
- [`conv9`](conv9/README.md) is an on-demand convolution playground over twelve diverse one-minute sources. Its Tauri app independently selects both clips and exposes each method’s own continuous windows and sound-shaping parameters for multiresolution, sliding WOLA, dual evolving-IR, crossfaded chunks, or full linear convolution. Results exist only in memory for looping playback, waveform, and high-resolution spectrogram inspection.
