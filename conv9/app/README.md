...
The complex geometric method shares Full convolution's A/B offset and duration controls, then adds A/B balance and complex power. It evaluates `X^(κ(1−α))Y^(κα)` with phase unwrapped across frequency, so every creative control acts multiplicatively on magnitude and phase together; it never performs an additive or magnitude-only crossfade.
