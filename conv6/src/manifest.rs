...
pub fn load_manifest(path: &Path) -> Result<Vec<SourceEntry>> {
    ...
    for domain in REQUIRED_DOMAINS {
        ...
        let domain_short = matching
            .iter()
            .filter(|entry| entry.seconds <= 35.0)
            .count();
        let domain_long = matching.iter().filter(|entry| entry.seconds > 35.0).count();
...
mod tests {
    ...
    fn checked_in_manifest_has_the_required_shape() {
        ...
        for domain in REQUIRED_DOMAINS {
            ...
            assert_eq!(
                matching.iter().filter(|entry| entry.seconds > 35.0).count(),
                1
            );
        }
    }
