...
4. **Independent chunks** convolves lockstep chunks and joins them with equal-power fades. It exposes A/B windows, input taper, overlap, and onset-to-tail crop position; the UI also reports the resulting overlap duration.
...
6. **Dry A / dry B** play either complete conditioned input exactly as it enters the convolution engine. They apply no convolution, output saturation, or second normalization pass and have no controls.
