...
2. **Source-filter vocoder** uses a 2,048-sample STFT at an 87.5% overlap to keep A's complex phase, timing, amplitude motion, and protected attacks while a normalized band-power B/A envelope ratio transfers B's time-varying timbre. Gaussian frequency smoothing and a 30 ms log-ratio smoother suppress circular-filter and hop-rate artifacts. Each shaped frame is power-normalized back to A, so level steps in B cannot become gain pulses. Transient detection learns A's stationary spectral-flux floor instead of misclassifying steady noise as a stream of attacks. Its compact panel exposes transfer strength, envelope width, and transient protection; it has no long-window controls.
3. **Latent convolution bank** self-supervises eight reusable spectro-temporal response patterns and sparse activation streams from each source using nonnegative matrix factor deconvolution. Soft activity routing drives A's learned response bank with B's latent events without assuming instruments, voices, or other fixed source categories. A capacity-limited residual retains stochastic structure the bank cannot economically explain. B supplies phase and excitation timing, while A's learned 40–250 ms responses may redistribute that energy into their own attacks and decays. Its controls are transfer strength and response memory, defaulting to full transfer and 170 ms.
4. **Moving impulse response** keeps A continuous and treats a centered segment around the corresponding point in B as a causal FIR reverb. It captures B snapshots at the selected update spacing, energy-normalizes them, then interpolates a coherence-aware filter for every 1,024-sample A block. A snapshot always retains the requested length: portions before B begins or after B ends are true silence rather than causing the window to slide inward. Identical neighboring IRs do not swell; unrelated IRs retain equal energy; real negative correlation may cancel without causing compensating gain. IRs through 2 seconds use uniform 1,024-sample partitioned FFT convolution. Longer IRs use an algebraically equivalent grouped-snapshot FFT path whose reusable workers and output merge are parallel and memory-bounded. Both paths preserve every A sample and every filter tail. Its panel exposes 0.05–30.00 seconds of IR length, update spacing, and IR taper.
...
7. **Complex geometric morph** uses the same independently selectable A/B segments but combines their complete spectra as `Z = X^(κ(1−α)) Y^(κα)`. Balance `α` moves geometrically between the sources without additive mixing. Complex power `κ` scales logarithmic magnitude and frequency-unwrapped phase together across `0.25–4.00`: `κ=1` is a geometric morph, equal balance at `κ=2` is ordinary convolution, values below `2` are rooted products, and values above `2` intensify through the squared complex product at `κ=4`. The output retains the same selected `A + B − 1` timeline as full convolution.
8. **Dry A / dry B** play either complete conditioned input exactly as it enters the convolution engine. They apply no convolution, output saturation, or second normalization pass and have no controls.
...
The parameter audit intentionally leaves the aligned A/B scan, FFT planning, phase unwrapping, output RMS, DC/high-pass conditioning, saturation, peak ceiling, and final edge fade automatic. They are implementation or safety invariants rather than creative controls. Every exposed parameter changes a distinct window, input taper, overlap, crop, carrier, segment, complex exponent, or synthesis decision and has a specific hover tooltip.

Convolution output is never forced back to the 61-second input duration. Windowed convolution retains its physical two-source timeline plus the complete first/last local support—about 132 seconds for 5×5-second windows and 122.5 seconds for 0.25×0.25-second windows. Moving IR produces `A duration + IR length - 1 sample`, including the final reverb tail. Chunks retain the actual union of their crossfaded blocks. Full convolution and complex geometric morphing have `A duration + B duration - 1 sample` frames, or just under 122 seconds when both complete clips are selected. The vocoder deliberately follows A's timeline, while the latent convolution bank follows B's 61-second excitation timeline and allows A's learned local responses to ring within it. The UI transport and visualizations follow the actual rendered length.
...
The browser suite checks the control model, stale-selection behavior, looping, visualizations, transport, and fixed-viewport layout. Rust regressions exercise independent noise, correlated tones, identical and opposite-polarity grains, sparse impulses, silence, slow ramps, unequal windows down to the scaled equivalent of 0.1×5 seconds, non-divisible timelines, power ripple, hop-phase modulation, and seam derivatives. Analytic complex-geometric tests use scaled and delayed impulses to verify source endpoints, logarithmic amplitude, interpolated phase delay, rooted powers, ordinary-convolution equivalence, and identical-spectrum invariance. The native suite drives the real Tauri app, renders both windowed and full convolution through Rust, records an isolated audio sink, and measures the captured PCM.
