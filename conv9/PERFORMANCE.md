...
| Latent convolution bank, 61 s | 164 ms, weak local LPC ratio | 212 ms, 8 learned response slots | 1.29× more work |
...
The latent convolution bank replaces the weak LPC ratio with self-supervised nonnegative matrix factor deconvolution. It compresses 1,024-frame, 50%-overlapped spectra into 48 logarithmic bands, then alternates six multiplicative-update passes over eight sparse response/activation slots for A and B. Rayon parallelizes independent band reconstruction, component updates, routing, and inverse transforms. Sinkhorn-scaled activity matching routes B's learned slots into A's learned 40–250 ms response patterns, while an explicit residual is capped at 25% so unexplained stochastic material survives without taking over the model. Synthesis retains B's phase where present, coherently propagates phase through learned decays, and preserves B's total rather than frame-local energy so A's response can outlive an excitation. The 61-second real-source DSP takes 212 ms and measured 78.7% movement toward A's spectral profile with 0.949 correlation to B's activity envelope.
