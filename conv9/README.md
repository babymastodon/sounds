...
2. **Source-filter vocoder** uses a 1,024-sample STFT at an 87.5% overlap to keep A's complex phase, timing, amplitude motion, and protected attacks while a frequency-smoothed B/A envelope ratio transfers B's time-varying timbre. Its compact panel exposes transfer strength, envelope width, and transient protection; it has no long-window controls.
...
Convolution output is never forced back to the 61-second input duration. Windowed convolution retains its physical two-source timeline plus the complete first/last local support—about 132 seconds for 5×5-second windows and 122.5 seconds for 0.25×0.25-second windows. Chunks retain the actual union of their crossfaded blocks. Full convolution has `A duration + B duration - 1 sample` frames, or just under 122 seconds when both complete clips are selected. The vocoder deliberately follows A's timeline and is exactly 61 seconds. The UI transport and visualizations follow the actual rendered length.
