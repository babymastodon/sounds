...
struct VerificationReport {
    ...
    rain_source_count: usize,
    sports_source_count: usize,
    long_instrument_source_count: usize,
    speeches_source_count: usize,
    train_ambient_source_count: usize,
    walking_source_count: usize,
...
pub fn render_matrix(options: RenderOptions) -> Result<()> {
    ...
    let pool = rayon::ThreadPoolBuilder::new()
        ...
        .thread_name(|index| format!("conv5-{index}"))
        .build()?;
...
fn verify_loaded(
    ...
    let report = VerificationReport {
        ...
        industrial_source_count: sources
            ...
            .count(),
        rain_source_count: sources
            .iter()
            .filter(|source| source.domain == "rain")
            .count(),
        sports_source_count: sources
            .iter()
            .filter(|source| source.domain == "sports")
            .count(),
        long_instrument_source_count: sources
            .iter()
            .filter(|source| source.domain == "long_instrument")
            .count(),
        speeches_source_count: sources
            .iter()
            .filter(|source| source.domain == "speeches")
            .count(),
        train_ambient_source_count: sources
            .iter()
            .filter(|source| source.domain == "train_ambient")
            .count(),
        walking_source_count: sources
            .iter()
            .filter(|source| source.domain == "walking")
            .count(),
