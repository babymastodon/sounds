...
3. **Predictive resonator bank** fits stable, 32-pole causal all-pole models in overlapping 8,192-sample local analysis windows. It removes B's local predictable response to recover an innovation signal, then resynthesizes that signal through A's corresponding local model. Transfer moves from an exact B identity transform to A's evolving modeled spectral body; ring controls pole damping. Root-Hann overlap-add preserves B's events and exact 61-second timeline.
